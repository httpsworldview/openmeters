...
use iced_widget::core::{
    self, Clipboard, Element, Layout, Padding, Point, Rectangle, Shell, Size, Vector,
};
    ...
    Renderer: core::Renderer,
{
    ...
    pub(crate) fn mouse_interaction(
        ...
        let mut interaction = if let Some(controls) = &self.controls {
            let controls_layout = children.next().unwrap();
        ...
        interaction = interaction.max(self.content.as_widget().mouse_interaction(
            &tree.children[0],
            title_layout,
            cursor,
            viewport,
    ...
    let (title_slice, rest) = tree.children.split_at_mut(1);
    let title_tree = &mut title_slice[0];
    let (controls_slice, rest) = rest.split_at_mut(1);
    let controls_tree = &mut controls_slice[0];
    let compact_tree = &mut rest[0];
        ...
        if let Some(controls) = self.controls.as_mut() {
            ...
            if title_layout.bounds().width + controls_layout.bounds().width > padded.bounds().width
            {
                if let Some(compact) = controls.compact.as_mut() {
                    ...
                    match compact.as_widget_mut().overlay(
                        compact_tree,
                        compact_layout,
                        renderer,
                        translation,
                    ) {
                        Some(overlay) => return Some(overlay),
                        None => {}
                    }
                } else {
                    show_title = false;

                    if let Some(overlay) = controls.full.as_widget_mut().overlay(
                        controls_tree,
                        controls_layout,
                        renderer,
                        translation,
            ...
            } else if let Some(overlay) = controls.full.as_widget_mut().overlay(
                controls_tree,
                controls_layout,
                renderer,
                translation,
